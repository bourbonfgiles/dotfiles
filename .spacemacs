...
This function should only modify configuration layer settings."
  (setq-default
   ...
   dotspacemacs-additional-packages '(helm-posframe
                                      centaur-tabs
                                      doom-themes
                                      gruvbox-theme
                                      dracula-theme
...
before packages are loaded."
  ...
  (with-eval-after-load 'treemacs
    (setq treemacs-position 'left) ;; position on left side
    (setq treemacs-width 30) ;; sidebar width
    (treemacs-follow-mode t) ;; follow current buffer
    (treemacs-filewatch-mode t) ;; auto-refresh
    (treemacs-git-mode 'extended)) ;; show git status

  ;; Auto-open Treemacs on startup with current directory
  (add-hook 'window-setup-hook
            (lambda ()
              (treemacs-add-and-display-current-project-exclusively)))


  ;; jj to escape from insert mode (like Neovim)
  ...
  ;; Enable centaur-tabs for buffer tabs at top
  ...
  ;; Helm charts - treat as YAML with special handling
  (with-eval-after-load 'yaml-mode
    (add-to-list 'auto-mode-alist '("/templates/.*\\.yaml\\'" . yaml-mode))
    (add-to-list 'auto-mode-alist '("/templates/.*\\.tpl\\'" . yaml-mode))
    (add-to-list 'auto-mode-alist '("\\.helmignore\\'" . gitignore-mode))
    (add-to-list 'auto-mode-alist '("Chart\\.yaml\\'" . yaml-mode))
    (add-to-list 'auto-mode-alist '("values.*\\.yaml\\'" . yaml-mode)))

  ;; OpenTofu support - use tofu command if available
  (with-eval-after-load 'terraform-mode
    (when (executable-find "tofu")
      (setq terraform-format-on-save-command "tofu fmt")))

  ;; Disable LSP auto-install prompts
  (setq lsp-enable-suggest-server-download nil)

  ;; Use ripgrep for project search with substring matching
  (with-eval-after-load 'helm-ag
    (when (executable-find "rg")
      (setq helm-ag-base-command "rg --no-heading --smart-case --line-number")
      (setq helm-ag-fuzzy-match t)))

  ;; Helm-posframe for floating window (like Telescope)
  (require 'helm-posframe)
  (helm-posframe-enable)
  
  (with-eval-after-load 'helm-posframe
    ;; Set posframe parameters for telescope-like appearance
    (setq helm-posframe-poshandler 'posframe-poshandler-frame-center)
    (setq helm-posframe-width 0.8)  ;; 80% of frame width
    (setq helm-posframe-height 0.7) ;; 70% of frame height
    (setq helm-posframe-parameters
          '((left-fringe . 8)
            (right-fringe . 8)
            (internal-border-width . 2)))
    
    ;; Always show preview as you navigate
    (setq helm-follow-mode-persistent t))
  
  ;; Auto-enable follow mode for file finding
  (add-hook 'helm-after-initialize-hook
            (lambda ()
              (when (or (string-match "Find Files" (buffer-name))
                        (string-match "Projectile" (buffer-name)))
                (helm-follow-mode 1))))
  )
...
This function is called at the very end of Spacemacs initialization."
  (custom-set-variables
   ...
   '(package-selected-packages
     '(ace-link aggressive-indent aio all-the-icons amx atom-one-dark-theme
                auto-compile auto-highlight-symbol auto-yasnippet autothemer
                avy-jump-helm-line browse-at-remote centaur-tabs
                centered-cursor-mode clean-aindent-mode code-cells code-review
                column-enforce-mode company-terraform company-web counsel
                counsel-css counsel-projectile cython-mode define-word devdocs
                diff-hl diminish dired-quick-sort disable-mouse docker
                dockerfile-mode dotenv-mode dracula-theme drag-stuff dumb-jump eat
                ...
                gitignore-templates golden-ratio google-translate gruvbox-theme
                haml-mode hcl-mode helm helm-ag helm-c-yasnippet helm-comint
                helm-company helm-core helm-css-scss helm-descbinds helm-ls-git
                ...
                indent-guide info+ inspector ivy ivy-avy ivy-hydra ivy-purpose
                ivy-xref ivy-yasnippet js-doc js2-mode js2-refactor json-mode
                json-navigator json-reformat json-snatcher link-hint live-py-mode
                livid-mode load-env-vars lorem-ipsum lsp-ivy lsp-origami
                lsp-treemacs lsp-ui macrostep markdown-toc monokai-theme
                multi-line multi-term multi-vterm multiple-cursors mwim nameless
                nodejs-repl nord-theme npm-mode open-junk-file org-superstar
                ...
                term-cursor terminal-here terraform-mode toc-org treemacs-evil
